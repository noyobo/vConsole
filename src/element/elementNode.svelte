  ...
  >
    ...
    {:else if node.nodeType === Node['ELEMENT_NODE']}
      <span class="vcelm-node" on:click={onTapNode}>
        &lt;{node.nodeName}
        {#if node.className || node.attributes.length}
          <i class="vcelm-k">
            {#each node.attributes as attr}
              {#if attr.name.indexOf('meta:') == -1}
                &nbsp;
                {#if attr.value !== ''}
                  {attr.name}="<i class="vcelm-v">{attr.value}</i>"{:else}{attr.name}{/if}{/if}{/each}</i
          >{/if}{#if node._isNullEndTag}/{/if}&gt;</span
      >
      {#if node.childNodes.length > 0}
        {#if !node._isExpand}...{:else}
          {#each node.childNodes as childNode}
            <svelte:self node={childNode} on:toggleNode />
          {/each}
        {/if}
      {/if}
      {#if !node._isNullEndTag}<span class="vcelm-node">&lt;/{node.nodeName}&gt;</span>{/if}
